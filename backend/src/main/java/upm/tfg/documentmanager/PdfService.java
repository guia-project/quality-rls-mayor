...
public class PdfService {
    ...
    public  ByteArrayInputStream exportResultPdf(List<ValidationResult> results){
        ...
        document.add(new Paragraph(" "));
        Table table = new Table(UnitValue.createPercentArray(new float[]{2, 2, 4, 2,5})).useAllAvailableWidth();

        for (String header : new String[]{"Nombre", "Tipo", "Descripción", "Resultado","Resultado Select"}) {
        ...
        for (ValidationResult r : results) {
            ...
            String selectResults =formatQueryResults(r);
            table.addCell(selectResults);
        }
        document.add(table);
        document.close();
